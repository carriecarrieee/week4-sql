...
SELECT models.name, brands.name, brands.headquarters FROM brands JOIN models ON (brands.brand_id=models.brand_id) WHERE models.name='Mustang';
...
SELECT * FROM models WHERE 15 <= model_id AND model_id <= 24 ORDER BY year;
...
select b.name,
m.year, m.name
from brands as b join models as m on b.brand_id=m.brand_id order by b.name;
...
select b.name, m.name, m.year, b.discontinued, b.discontinued - m.year AS years_until_brand_discontinued FROM models AS m JOIN brands AS b ON b.brand_id=m.brand_id WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
...
SELECT models.name, models.year FROM models JOIN brands ON (models.year=brands.founded);